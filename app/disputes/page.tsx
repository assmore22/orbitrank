...
import { Banner, Empty, GhostOrbit, Hex, ExtLink, StatusChip } from "@/components/ui";
import { useTx } from "@/components/Tx";
import { useLoader } from "@/lib/hooks";
import { getOpenChallenges, getOpenAppeals, hasContract } from "@/lib/orbitrank";
...
export default function DisputesPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <div className="label flex items-center gap-2"><FontAwesomeIcon icon={faScaleBalanced} /> Adjudication ring</div>
        <h1 className="mt-1 text-2xl font-semibold tracking-tight">Challenges &amp; appeals</h1>
        <p className="mt-1 max-w-2xl text-sm text-muted">Open challenges and appeals are re-adjudicated by the AI reviewer against the project rubric and fresh evidence.</p>
      </div>
      ...
      {tab === "challenges" ? (
        challenges.loading && !challenges.data ? <GhostOrbit size={140} /> :
        ...
        (challenges.data?.length ?? 0) === 0 ? <Empty icon={faGavel} title="No open challenges" hint="Resolved challenges are recorded in each project’s audit trail." /> :
          ...
          {challenges.data!.map((c) => (
            <div key={c.challengeId} className="panel space-y-2 p-4">
              ...
              <div className="flex items-center justify-between border-t border-line pt-2 text-xs text-muted">
                ...
                <div className="flex items-center gap-2"><Link href={`/project/${c.projectId}`} className="text-primary hover:underline">project #{c.projectId} <FontAwesomeIcon icon={faArrowRight} className="h-2.5 w-2.5" /></Link><button className="btn btn-primary btn-xs" disabled={busy} onClick={() => resolveC(c.challengeId)}><FontAwesomeIcon icon={faBolt} className="h-3 w-3" /> Resolve</button></div>
              </div>
            </div>
          ))}
        </div>
      ) : (
        appeals.loading && !appeals.data ? <GhostOrbit size={140} /> :
        ...
        (appeals.data?.length ?? 0) === 0 ? <Empty icon={faScaleBalanced} title="No open appeals" hint="Appeals can be filed on rejected, partial, gaming, challenged or accepted claims." /> :
        <div className="space-y-3">
          {appeals.data!.map((a) => (
            <div key={a.appealId} className="panel space-y-2 p-4">
              ...
              <div className="flex items-center justify-between border-t border-line pt-2 text-xs text-muted">
                ...
                <div className="flex items-center gap-2"><Link href={`/project/${a.projectId}`} className="text-primary hover:underline">project #{a.projectId} <FontAwesomeIcon icon={faArrowRight} className="h-2.5 w-2.5" /></Link><button className="btn btn-primary btn-xs" disabled={busy} onClick={() => resolveA(a.appealId)}><FontAwesomeIcon icon={faBolt} className="h-3 w-3" /> Resolve</button></div>
              </div>
            </div>
          ))}
        </div>
